   ...
   export default {
     ...
     computed: {
       ...
       plugins() {
         ...
         if (this.scale) {
           this.plugin.push('ToolBar');
         }
         if (this.geolocation) {
           this.plugin.push('Geolocation');
         }
     ...
     methods: {
       addPlugins() {
         let _notInjectPlugins = this.plugins.map((_plu) => _plu['pName']);
         if (_notInjectPlugins.length === 0) {
           return;
         }
         if (!_notInjectPlugins || !_notInjectPlugins.length) return this.addMapControls();
         return AMap.plugin(_notInjectPlugins, this.addMapControls);
       },
       ...
       createMap() {
         this._loadPromise.then(() => {
           ...
           CONST.MAP_EVENTS.forEach((ev) => {
             AMap.event.addListener(this.$amap, ev, () => {
